...
//            List<ProductResponse> response = listProduct.stream().map(productService::todo).toList();
//            return ResponseEntity.ok(Map.of("message", "Lấy danh sách product thành công", "response", response));
//        } catch (Exception e) {
//            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Lỗi khi lấy danh sách product: " + e.getMessage());
//        }
//    }
    ...
    public ResponseEntity<?> getProductById(@PathVariable("id") int id) {
        try {
            ...
            return ResponseEntity.ok(Map.of("message", "Lấy product thành công", "response", productResponse));
        } catch (Exception e) {
    ...
    public ResponseEntity<?> updateProduct(@PathVariable("id") int id, @RequestBody ProductDTO productDTO) {
        try {
            ...
            return ResponseEntity.ok(Map.of("message", "Cập nhật product thành công"));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Lỗi khi cập nhật product: " + e.getMessage());
        }
    }
    ...
    public ResponseEntity<?> deleteProduct(@PathVariable("id") int id) {
        try {
            ...
            return ResponseEntity.ok(Map.of("message", "Xóa product thành công"));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Lỗi khi xóa product: " + e.getMessage());
        }
    }
